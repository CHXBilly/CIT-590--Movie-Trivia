...
class MovieTriviaTest {
	...
	void testInsertActor() {
		...
		// try to insert new actor with new movies
		// Bradley Cooper test case
		mt.insertActor("   Bradley Cooper", new String[] { "   American Sniper", "  A Star is Born" }, movieDB.getActorsInfo());
		assertEquals(8, movieDB.getActorsInfo().size(),
				"After inserting an actor, the size of actorsInfo should have increased by 1.");
		assertEquals("bradley cooper", movieDB.getActorsInfo().get(movieDB.getActorsInfo().size() - 1).getName(),
				"After inserting actor \"   Bradley Cooper   \", the name of the last actor in actorsInfo should be \"bradley cooper\".");
		assertEquals(2, movieDB.getActorsInfo().get(movieDB.getActorsInfo().size() - 1).getMoviesCast().size(),
				"Actor \"bradley cooper\" should have 2 movies in their moviesCasted list.");
		assertEquals("american sniper",
				movieDB.getActorsInfo().get(movieDB.getActorsInfo().size() - 1).getMoviesCast().get(0),
				"\"american sniper\" should be the first movie in Bradley Cooper's moviesCasted list.");
				
				// try to insert new actor with new movies
				// Jackie Chan test case
		mt.insertActor("JACKIE CHAN   ", new String[] { "RUSH HOUR  ", "THE FOREIGNER " }, 
				movieDB.getActorsInfo());
		assertEquals(9, movieDB.getActorsInfo().size(),
				"After inserting an actor, the size of actorsInfo should have increased by 1.");
		assertEquals("jackie chan", movieDB.getActorsInfo().get(movieDB.getActorsInfo().size() - 1).getName(),
				"After inserting actor \"JACKIE CHAN   \", the name of the last actor in actorsInfo should be \"jackie chan\".");
		assertEquals(2, movieDB.getActorsInfo().get(movieDB.getActorsInfo().size() - 1).getMoviesCast().size(),
				"Actor \"jackie chan\" should have 2 movies in their moviesCasted list.");
		assertEquals("rush hour",
				movieDB.getActorsInfo().get(movieDB.getActorsInfo().size() - 1).getMoviesCast().get(0),
				"\"rush hour\" should be the first movie in Jackie Chan's moviesCasted list.");
						
		// try to insert existing actor with two movies (both being new)
		// Brad Pitt test case
		mt.insertActor("Brad PITT", new String[] { " MONEYBALL ", "  The Big Short" }, movieDB.getActorsInfo());
		assertEquals(9, movieDB.getActorsInfo().size(),
				"Since \"brad pitt\" is already in actorsInfo, inserting \"Brad PITT\" again should not increase the size of actorsInfo.");
		
		// look up and inspect movies for existing actor
		assertEquals(4, mt.selectWhereActorIs("Brad PITT", movieDB.getActorsInfo()).size(),
				"After inserting Brad Pitt again with 2 movies, both of which are not on the list yet, the number of movies \"brad pitt\" appeared in should be 4.");
		
		assertTrue(mt.selectWhereActorIs("Brad PITT", movieDB.getActorsInfo()).contains("moneyball"),
				"After inserting Brad Pitt again with a new Movie \" MONEYBALL \", \"moneyball\" should appear as one of the movies he has appeared in.");
		assertTrue(mt.selectWhereActorIs("Brad PITT", movieDB.getActorsInfo()).contains("the big short"),
				"After inserting Brad Pitt again with a new Movie \"  The Big Short\", \"the big short\" should appear as one of the movies he has appeared in.");
		
		// try to insert existing actor with three movies (two being new)
		// Robin Williams test case
		mt.insertActor("  ROBIN williams  ", new String[] { " good WILL hunting", "dead poets SOCIETY ", " POPEYE " },movieDB.getActorsInfo());
		assertEquals(9, movieDB.getActorsInfo().size(),
				"Since \"robin williams\" is already in actorsInfo, inserting \"  ROBIN williams  \" again should not increase the size of actorsInfo.");
				
				// look up and inspect movies for existing actor
		assertEquals(3, mt.selectWhereActorIs("  ROBIN williams  ", movieDB.getActorsInfo()).size(),
				"After inserting Robin Williams again with 3 movies, two of which are not on the list yet, the number of movies \"robin williams\" appeared in should be 3.");
		assertTrue(mt.selectWhereActorIs("  ROBIN williams  ", movieDB.getActorsInfo()).contains("good will hunting"),
				"After inserting Robin Williams again with a new Movie \" good WILL hunting\", \"good will hunting\" should appear as one of the movies he has appeared in.");
		assertTrue(mt.selectWhereActorIs("  ROBIN williams  ", movieDB.getActorsInfo()).contains("dead poets society"),
				"After inserting Robin Williams again with a new Movie \"dead poets SOCIETY \", \"dead poets society\" should appear as one of the movies he has appeared in.");	
